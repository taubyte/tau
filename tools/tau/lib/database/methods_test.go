...
func TestList_TCCFixture(t *testing.T) {
	...
	// TCC fixture global: test_database1
	assert.Equal(t, len(names), 1)
	assert.Equal(t, names[0], "test_database1")
}
...
func TestListResources_TCCFixture(t *testing.T) {
	...
	assert.Equal(t, resources[0].Name, "test_database1")
	assert.Equal(t, resources[0].Description, "a database for users")
}
...
func TestList_TCCFixture_WithApp(t *testing.T) {
	...
	names, err := List()
	assert.NilError(t, err)
	// test_app1 has test_database2
	assert.Equal(t, len(names), 1)
	assert.Equal(t, names[0], "test_database2")
}
...
func TestListResources_TCCFixture_WithApp(t *testing.T) {
	...
	assert.NilError(t, err)
	assert.Equal(t, len(resources), 1)
	assert.Equal(t, resources[0].Name, "test_database2")
}
