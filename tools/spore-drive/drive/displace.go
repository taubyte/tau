...
import (
	...
	"github.com/taubyte/tau/tools/spore-drive/course"
	"gopkg.in/ini.v1"
	"gopkg.in/yaml.v3"
)
...
func (d *sporedrive) writeConfig(h remoteHost, shape string, w io.Writer) error {
	// TODO
	hc := d.parser.Hosts().Get(h.Host().Name())
	hshape := hc.Shapes().Get(shape)
	sc := d.parser.Shapes().Get(shape)
	lat, lng := hc.Location()
	...
	for _, sh := range d.parser.Cloud().P2P().Bootstrap().List() {
		for _, ht := range d.parser.Cloud().P2P().Bootstrap().Get(sh) {
			if ht == h.Host().Name() {
				continue
			}

			htc := d.parser.Hosts().Get(ht)
			bt := htc.Shapes().Get(sh)
			shid := bt.Id()
			shMainPort := int(d.parser.Shapes().Get(sh).Ports().Get("main"))
...
func (d *sporedrive) displaceHandler(hypha *course.Hypha, progressCh chan<- Progress) func(context.Context, host.Host) error {
	...
	return func(ctx context.Context, h host.Host) error {
		...
		hshapes := d.parser.Hosts().Get(h.Name()).Shapes().List()
