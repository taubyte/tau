...
func TestParserEdit(t *testing.T) {
	...
	assert.NilError(t, p.Shapes().Add("shape1").Services().Set("auth", "seer"))
	assert.NilError(t, p.Shapes().Add("shape1").Ports().Set("main", 4242))
	assert.NilError(t, p.Shapes().Add("shape1").Ports().Set("lite", 4262))

	assert.NilError(t, p.Shapes().Add("shape2").Services().Set("gateway", "patrick", "monkey"))
	assert.NilError(t, p.Shapes().Add("shape2").Ports().Set("main", 6242))
	assert.NilError(t, p.Shapes().Add("shape2").Ports().Set("lite", 6262))
	assert.NilError(t, p.Shapes().Add("shape2").Plugins().Set("plugin1@v0.1"))
	...
	assert.NilError(t, host1.Shapes().Add("shape1").GenerateKey())
	assert.NilError(t, host1.Shapes().Add("shape2").GenerateKey())
...
func TestParserSchema(t *testing.T) {
	...
	assert.Equal(t, p.Shapes().Get("shape2").Ports().Get("lite"), uint16(6262))
	...
	assert.Equal(t, host1.Shapes().Add("shape1").Key(), "CAESQIWC2KRhsEexLpN4DsJwki4S56IN5IreCANf89+F+OpTWn7Tf+RwZnUbiZYdxsTFrbBJQ9S+A0oFp8a1SSAN2EE=")
	assert.Equal(t, host1.Shapes().Add("shape1").Id(), "12D3KooWFud2Y9UuRW6Si2whRoyu9AXJLEgFed6xhabsyVe8pHck")

	assert.Equal(t, host1.Shapes().Add("shape2").Key(), "CAESQHLGyFbnI2GP7e3Gib9ut7IFDxrkbTbs7LFAJYhe0w0LXEtYrH7HyODglOFY3oXQ+kCfoFcvqvZnAD6K5UavO2c=")
...
func TestParserDelAppendClear(t *testing.T) {
	...
	assert.NilError(t, p.Hosts().Get("host1").Shapes().Delete("shape1"))
	assert.DeepEqual(t, p.Hosts().Get("host1").Shapes().List(), []string{"shape2"})
	assert.Equal(t, p.Hosts().Get("host1").Shapes().Get("shape1").Id(), "")

	assert.NilError(t, p.Hosts().Delete("host1"))
	assert.DeepEqual(t, p.Hosts().List(), []string{"host2"})

	assert.NilError(t, p.Hosts().Get("host2").Addresses().Delete("8.2.3.4/24"))
	assert.DeepEqual(t, p.Hosts().Get("host2").Addresses().List(), []string{"4.3.2.8/24"})

	assert.NilError(t, p.Hosts().Get("host2").Addresses().Append("8.2.3.4/24"))
	assert.DeepEqual(t, p.Hosts().Get("host2").Addresses().List(), []string{"4.3.2.8/24", "8.2.3.4/24"})
	assert.NilError(t, p.Hosts().Get("host2").Addresses().Clear())
	assert.DeepEqual(t, p.Hosts().Get("host2").Addresses().List(), []string{})

	assert.NilError(t, p.Shapes().Get("shape2").Ports().Delete("lite"))
	assert.DeepEqual(t, p.Shapes().Get("shape2").Ports().List(), []string{"main"})
