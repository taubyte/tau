...
type HostsParser interface {
	List() []string
	Get(string) HostParser
	Add(string) HostParser
	Delete(string) error
}
...
type HostShapesParser interface {
	List() []string
	Get(string) InstanceParser
	Delete(string) error
	Add(string) InstanceParser
}
...
func (h *hosts) Get(name string) HostParser {
	return &host{root: h.root, Query: h.Fork().Get(name)}
}

func (h *hosts) Add(name string) HostParser {
	return &host{root: h.root, Query: h.Fork().Get(name), mode: leafModeAdd}
}
...
func (hs *hostShapes) Get(s string) InstanceParser {
...
func (hs *hostShapes) Add(s string) InstanceParser {
	return &instShape{root: hs.root, Query: hs.Fork().Get(s), mode: leafModeAdd}
}
