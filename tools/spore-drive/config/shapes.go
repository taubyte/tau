...
type ShapesParser interface {
	List() []string
	Get(string) ShapeParser
	Add(string) ShapeParser
	Delete(string) error
}
...
func (s *shapes) Get(name string) ShapeParser {
	return &shape{root: s.root, Query: s.Fork().Get(name)}
}

func (s *shapes) Add(name string) ShapeParser {
	return &shape{root: s.root, Query: s.Fork().Get(name), mode: leafModeAdd}
}
