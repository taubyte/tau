...
func createConfig() (afero.Fs, config.Parser) {
	...
	privKeyFile, _ := p.Auth().Get("withkey").Open()
	io.Copy(privKeyFile, bytes.NewBuffer(privKeyData))
	privKeyFile.Close()

	p.Shapes().Add("shape1").Services().Set("auth", "seer")
	p.Shapes().Add("shape1").Ports().Set("main", 4242)
	p.Shapes().Add("shape1").Ports().Set("lite", 4262)

	p.Shapes().Add("shape2").Services().Set("gateway", "patrick", "monkey")
	p.Shapes().Add("shape2").Ports().Set("main", 6242)
	p.Shapes().Add("shape2").Ports().Set("lite", 6262)
	p.Shapes().Add("shape2").Plugins().Set("plugin1@v0.1")
	...
	host1.Shapes().Add("shape1").GenerateKey()
	host1.Shapes().Add("shape2").GenerateKey()
	...
	host2.Shapes().Add("shape1").GenerateKey()
	host2.Shapes().Add("shape2").GenerateKey()

	p.Cloud().P2P().Bootstrap().Append("shape1", host2.Shapes().Get("shape1").Id(), host1.Shapes().Get("shape1").Id())
	p.Cloud().P2P().Bootstrap().Append("shape2", host2.Shapes().Get("shape2").Id(), host1.Shapes().Get("shape2").Id())
