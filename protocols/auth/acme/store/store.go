...
var (
	MinPeers = 0
	MaxPeers = 4
	logger   = logging.Logger("auth.acme.store")
)
...
func New(ctx context.Context, node peer.Node, cacheDir string, errCacheMiss error) (*Store, error) {
	...
	if err != nil {
		logger.Error(fmt.Sprintf("ACME Store creation failed: %s", err.Error()))
		return nil, err
	}
...
func (d *Store) Get(ctx context.Context, name string) ([]byte, error) {
	logger.Debug(fmt.Sprintf("Getting `%s`", name))
	defer logger.Debug(fmt.Sprintf("Getting `%s` done", name))
	...
	if err != nil {
		if d.errCacheMiss != nil && err.Error() == d.errCacheMiss.Error() {
			logger.Debug(fmt.Sprintf("Cache miss for `%s` returning ErrCacheMiss", name))
			return nil, d.errCacheMiss
		}
		logger.Error(fmt.Sprintf("Getting `%s` failed: %s", name, err.Error()))
		return nil, err
	}

	pem, err := maps.ByteArray(res, dataKey)
	if err != nil {
		logger.Error(fmt.Sprintf("Reading PEM error: %s", err.Error()))
		return nil, err
	}

	logger.Debug(fmt.Sprintf("Getting `%s` = %v", name, pem))
...
func (d *Store) Put(ctx context.Context, name string, data []byte) error {
	logger.Debug(fmt.Sprintf("Storing `%s`", name))
	defer logger.Debug(fmt.Sprintf("Storing `%s` done", name))
	...
	if err != nil {
		logger.Error(fmt.Sprintf("Storing `%s` error: %s", name, err.Error()))
	}
	return err
}
...
func (d *Store) Delete(ctx context.Context, name string) error {
	logger.Debug(fmt.Sprintf("Deleteing `%s`", name))
	defer logger.Debug(fmt.Sprintf("Deleteing `%s` done", name))
	...
	if strings.HasSuffix(name, "+token") == true || strings.HasSuffix(name, "+rsa") == true || strings.HasSuffix(name, "+key") == true || strings.HasSuffix(name, ".key") == true {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("Deleting `%s` error: %s", name, err.Error()))
		}
		return err
	}
