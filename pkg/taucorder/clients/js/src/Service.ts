...
export class Service {
  ...
  private async downloadAndExtractBinary(): Promise<void> {
    ...
    const assetName = `taucorder-service_${version}_${currentOs}_${currentArch}.tar.gz`;
    const assetUrl = `https://github.com/taubyte/taucorder/releases/download/v${version}/${assetName}`;

    console.log(
      `Downloading taucorder service v${version} for ${currentOs}/${currentArch}...`
    );
    ...
    return new Promise((resolve, reject) => {
      writer.on("finish", async () => {
        progressBar.stop();
        console.log(`\nExtracting taucorder service v${version}...`);
        await tar.x({
          file: tarPath,
          C: this.binaryDir,
        });
