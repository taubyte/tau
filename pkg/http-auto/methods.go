...
func (s *Service) SetOption(opt interface{}) error {
	...
	case autoOptions.OptionAutoTrust:
		s.autoTrustDomain = opt.Fn
	case autoOptions.OptionSkipDomainProof:
		s.skipDomainProof = opt.Fn
	case autoOptions.OptionClientNode:
		s.clientNode = opt.Node
	case options.OptionACME:
		s.acme = &opt
	case options.OptionACMECARoots:
		s.acmeCARoots = opt.Roots
	case options.OptionACMECASkipVerify:
		s.acmeCASkipVerify = opt.Skip
	}
...
func newAuto(ctx context.Context, node peer.Node, opts ...options.Option) (*Service, error) {
	...
	s := &Service{Service: _s}

	if err := options.Parse(s, opts); err != nil {
		return nil, err
	}

	// Single-node deployments and tests don't supply a separate clientNode.
	if s.clientNode == nil {
		s.clientNode = node
	}

	s.authClient, err = authP2P.New(s.Context(), s.clientNode)
	if err != nil {
		return nil, fmt.Errorf("failed creating auth client with %v", err)
	}

	s.tnsClient, err = tnsP2P.New(s.Context(), s.clientNode)
	if err != nil {
		return nil, fmt.Errorf("failed creating tns client with %v", err)
	}

	cacheDir, err := s.clientNode.NewFolder("acme")
	if err != nil {
		logger.Error("creating acme cache foler failed with ", err)
		return nil, err
	}

	s.certStore, err = acmeStore.New(s.clientNode.Context(), s.clientNode, cacheDir.Path())
	if err != nil {
		logger.Error("new Auto HTTP: ", err)
		return nil, err
	}
...
func (s *Service) validateFQDN(host string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed validateFromTns for %s with %v", host, err)
	}

	// Generated subdomains skip the per-project TXT proof — they're issued
	// by tau itself, not bound to an external project owner.
	if s.skipDomainProof == nil || !s.skipDomainProof(host) {
		if projectId == "" {
			s.negativeCache.Set(host, true, NegativeTTL)
		...
		if err != nil {
			s.negativeCache.Set(host, true, NegativeTTL)
			return fmt.Errorf("failed txt lookup on %s with %v", host, err)
		}
	}
...
func (s *Service) Start() {
	...
	m := &autocert.Manager{
		...
		HostPolicy: func(ctx context.Context, host string) error {
			...
			} else if s.autoTrustDomain == nil || !s.autoTrustDomain(host) {
	...
	if s.acme != nil {
		...
		if s.acmeCASkipVerify || s.acmeCARoots != nil {
			m.Client.HTTPClient = &http.Client{
				Transport: &http.Transport{
					TLSClientConfig: &tls.Config{
						InsecureSkipVerify: s.acmeCASkipVerify,
						RootCAs:            s.acmeCARoots,
					},
				},
			}
		}
