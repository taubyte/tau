...
func TestMessaging_GlobalMessaging(t *testing.T) {
	root := object.New[object.Refrence]()
	configRoot := object.New[object.Refrence]()
	configRoot.Set("id", "project-id-123")

	// Create messaging
	messagingConfig, _ := configRoot.CreatePath(string(specs.PathVariable))
	msgSel := messagingConfig.Child("msg-id-456")
	msgSel.Set("name", "myMessaging")

	ctx := transform.NewContext[object.Refrence](context.Background(), root, configRoot)
	ctx = ctx.Fork(configRoot)

	transformer := Messaging("main")
...
func TestMessaging_AppMessaging(t *testing.T) {
	root := object.New[object.Refrence]()
	configRoot := object.New[object.Refrence]()
	configRoot.Set("id", "project-id-123")
	...
	// Create messaging in app
	messagingConfig, _ := appObj.CreatePath(string(specs.PathVariable))
	msgSel := messagingConfig.Child("msg-id-999")
	msgSel.Set("name", "appMessaging")
...
func TestMessaging_WithExistingLinks(t *testing.T) {
