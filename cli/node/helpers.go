...
func createLiteNode(ctx context.Context, conf config.Config, shape, storagePath string) (peer.Node, error) {
	announce := conf.P2PAnnounce()
	if len(announce) == 0 {
		return nil, fmt.Errorf("p2p announce is empty")
	}
...
func createNodes(ctx context.Context, storagePath, shape string, conf config.Config) error {
	if len(conf.Services()) < 1 { // For elder nodes
		...
		n, err := peer.NewFull(ctx, storagePath, conf.PrivateKey(), conf.SwarmKey(), conf.P2PListen(), conf.P2PAnnounce(), true, peer.BootstrapParams{Enable: true, Peers: peerInfo})
		if err != nil {
		...
		// Non elder nodes
		n, err := config.NewNode(ctx, conf, storagePath)
		if err != nil {
		...
		// Create client node
		clientNode, err := createLiteNode(ctx, conf, shape, storagePath)
		if err != nil {
			return fmt.Errorf("creating client node failed with: %s", err)
		}
		conf.SetClientNode(clientNode)
	}
