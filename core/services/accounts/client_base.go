...
//
// The base surface is community: linkage is the access model. Any ee-only
// surface rides on the embedded eeSurface — empty in the community build, filled in
// under -tags ee (see client.go / client_ee.go).
type Client interface {
	...
	// Management surface — requires a Member session (per-account ops).
	Accounts() Accounts
	...
	// eeSurface is empty in the community build; the ee build fills it in.
	eeSurface
}
...
// Users is the linked-git-account surface for one Account. Linkage is the
// grant: Add gives the linked git user full access.
