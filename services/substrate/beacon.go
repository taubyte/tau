...
// TODO: REMOVE
func (srv *Service) startBeacon(cfg config.Config) (beacon *serviceBeacon, err error) {
	beacon = &serviceBeacon{Service: srv, config: cfg}
	...
	beacon.seerClient, err = seerClient.New(srv.ctx, clientNode, cfg.SensorsRegistry())
	if err != nil {
...
func (beacon *serviceBeacon) hostname() (err error) {
	announce := beacon.config.P2PAnnounce()
	if len(announce) == 0 {
	...
	// Start usage beacon (cluster is set by config.Validate, default "main")
	beacon.seerClient.Usage().AddService(seerIface.ServiceTypeSubstrate, map[string]string{"IP": addr, "cluster": beacon.config.Cluster()})
