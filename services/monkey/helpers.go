...
func (m *Monkey) storeLogs(r io.ReadSeeker) (string, error) {
	if _, err := r.Seek(0, io.SeekStart); err != nil {
		return "", fmt.Errorf("failed to seek logs: %w", err)
	}

	cid, err := m.Service.node.AddFile(r)
	if err != nil {
		return "", fmt.Errorf("failed to add logs to node: %w", err)
	}
...
func (m *Monkey) tryGetGitRepo(
	...
	for i := 0; i < GetGitRepoMaxRetries; i++ {
		...
		if err != nil {
			return gitRepo, fmt.Errorf("failed to fetch repository %d from auth: %w", repoID, err)
		}
