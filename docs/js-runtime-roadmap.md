...
- ◑ Next.js: static/pre-rendered + `GET` edge routes today; dynamic React SSR
  renders via `next-on-pages` + `--engine starlingmonkey` (POST edge routes
  pending — see §5).
...
### 5. Next.js on the richer engine — ◑ (dynamic SSR renders; POST pending)
A real Next.js 14 App Router app, built with `next-on-pages` and run via
`--engine starlingmonkey`, **renders a dynamic React SSR page end to end**
(`react-dom/server`, `force-dynamic`) plus `GET` edge API routes and prerendered
pages. Two shim stream-polyfills were needed for the StarlingMonkey build jco
ships — ReadableStream async iteration and byte-stream `tee()` — both of which
Next's streaming SSR exercises.

Remaining: **POST** to a Next.js edge API route traps inside Next's request
machinery (`IndirectCallToNull`); a general component POST-with-body works, so
it's Next-specific (likely another spot needing an engine/shim fix, or it clears
with a newer StarlingMonkey once WASI 0.2.10 ships in a wasmtime release — see §4).
